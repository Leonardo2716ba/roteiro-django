...
Este roteiro é um passo a passo para você construir, do zero, o projeto demo-django: um site simples de uma página, escrito em Django, estilizado com Tailwind CSS (via CDN), salvando dados em SQLite e executado dentro de um container Docker.

Ao final, você terá uma página inicial que lista mensagens cadastradas no banco através do painel administrativo do Django.

Sumário
Pré-requisitos
Criar a pasta do projeto
Declarar as dependências (requirements.txt)
Escrever o Dockerfile
Escrever o docker-compose.yml
Configurar .gitignore e .dockerignore
Gerar a estrutura do Django
Criar o app home
Ajustar core/settings.py
Definir o modelo Mensagem
Registrar o modelo no admin
Criar a view index
Configurar as URLs
Criar o template HTML com Tailwind
Aplicar as migrations e subir o servidor
Criar o usuário admin e cadastrar mensagens
Para experimentar em aula
1. Pré-requisitos
...
💡 Por que Docker? Para você não precisar instalar Python e Django direto na sua máquina. Tudo roda dentro de um container isolado.
...
O Dockerfile é a receita para construir a imagem do nosso container. Crie o arquivo Dockerfile (sem extensão) na raiz:
...
O docker-compose.yml orquestra o serviço (subir, expor portas, mapear pastas). Crie o arquivo na raiz:
...
volumes: .:/app — monta a pasta atual dentro de /app no container, então qualquer alteração no código aparece em tempo real (sem precisar rebuildar).
command — sobrescreve o CMD do Dockerfile: roda migrate (cria as tabelas) e depois sobe o servidor de desenvolvimento.
...
Aqui você usa o próprio Docker para rodar o django-admin sem precisar de Python instalado. Na raiz do projeto:

docker compose run --rm web django-admin startproject core .
Atenção ao ponto final — ele faz o Django criar a estrutura na pasta atual, sem criar uma pasta extra core/ envolvendo tudo.
...
8. Criar o app home
No Django, projeto é o pacote de configuração; apps são módulos com funcionalidades. Vamos criar um app chamado home:
...
Isso faz a Mensagem aparecer em /admin/ com uma lista mostrando título e data, e com uma caixa de busca por título e conteúdo.
...
<script src="https://cdn.tailwindcss.com"></script> carrega o Tailwind via CDN (sem precisar de build/npm).
{% if mensagens %}, {% for m in mensagens %}, {{ m.titulo }} são comandos da linguagem de templates do Django.
...
Construir a imagem (só na primeira vez, ou quando o Dockerfile mudar).
Aplicar as migrations no SQLite (db.sqlite3 é criado).
Iniciar o servidor em http://localhost:8000.
Abra http://localhost:8000 no navegador. Você deve ver a página com o cabeçalho "Olá, Django + Tailwind!" e a seção dizendo "Nenhuma mensagem ainda."
...
Clique em Mensagens → Adicionar.
Preencha título e conteúdo e salve.
Volte a http://localhost:8000/ — sua mensagem aparece na lista.
🎉 Parabéns! Você acabou de construir um site completo com modelo, view, template, banco de dados e admin.
...
Mudar o visual: edite as classes Tailwind em templates/home/index.html. Troque from-slate-900 via-indigo-900 por outro gradiente, por exemplo.

Criar uma nova página (ex.: /sobre/):

Adicione uma função em home/views.py:
def sobre(request):
    return render(request, "home/sobre.html")
Adicione a rota em home/urls.py:
path("sobre/", views.sobre, name="sobre"),
Crie templates/home/sobre.html.
Filtrar mensagens na view: troque Mensagem.objects.all() por Mensagem.objects.filter(titulo__icontains="oi") e veja só as que contêm "oi" no título.
